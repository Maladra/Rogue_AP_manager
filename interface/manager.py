...
# sudo airodump-ng wlan0mon -w test --output-format csv
...
class SelectBSSIDWindow(Gtk.Window):
    def __init__(self):
        ...
        self.add(self.box)
        ## List Model AP
        self.AP_liststore_model = Gtk.ListStore(str, str)
        self.AP_list = get_ap()
        ...
        self.column_apname = Gtk.TreeViewColumn("AP name", self.renderer_text, text=1)
        ## Add Column
    ...
    def on_button_refresh_clicked(self, widget):
        ...
        for AP in AP_list:
            self.AP_liststore_model.append(list(AP))
        #self.treeview_ap.set_model(AP_liststore_model)
        print("Refresh BSSID Liste")
